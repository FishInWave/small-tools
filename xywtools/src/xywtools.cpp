 ...
 */
    int AnalysJson::readJson(std::string jsonfile,std::vector<jsonanno::annobox>& boxlist)
    {
    Json::Value jsonroot;
 ...
 */
    int GetFilesName::getFilesName(std::string path,std::vector<std::string>& filesname){
        DIR *pDir;
        struct dirent* ptr;
        ...
        while((ptr = readdir(pDir)) != 0){
            ...
            filesname.push_back(path + "/" + ptr->d_name);
        }
        closedir(pDir);
        return 0;
    }
